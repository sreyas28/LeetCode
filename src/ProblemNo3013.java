...
public class ProblemNo3013 {
    public static void main(String[] args) {
        ProblemNo3013.Solution a = new ProblemNo3013().new Solution();
//        System.out.println(a.minimumCost(new int[]{1, 3, 2, 6, 4, 2}, 3, 3));
//        System.out.println(a.minimumCost(new int[]{10, 1, 2, 2, 2, 1}, 4, 3));
        System.out.println(a.minimumCost(new int[]{10,8,18,9}, 3, 1));
    }
    ...
    class Solution_ {
        public long minimumCost(int[] nums, int k, int dist) {
            ...
            for (int start = 1; start < nums.length; start++) {
                int end = Math.min(start + dist, nums.length - 1);
                if (end - start + 1 < k-1) break;

                List<Integer> list = new ArrayList<>();
                for (int putting = start + 1; putting <= end; putting++) {
                    list.add(nums[putting]);
                }
                Collections.sort(list);

                long tempSum = nums[start];
                for (int i = 0; i < k - 2; i++) tempSum += list.get(i);
